...
let currentReviewCount = initReviewCount; // 버튼을 누를때마다 current값 증가시켜서 추가 리뷰 가져오기
...
// 리뷰 평점 별로 표현
  ...
  const rating = ratingAvgVal; // 여기서는 서버에서 평점을 받아와서 사용

  // 별 아이콘을 표시할 div 요소를 가져오기
  const starIconsDiv = document.getElementById('starIcons');
  const starIconsDiv2 = document.getElementById('starIcons2');

  // 평점에 따라 별 아이콘을 생성하여 div에 추가
  function displayStarIcons(rating, targetDiv) {
    const fullStars = Math.floor(rating); // 평점의 정수 부분만 구하기
    const emptyStars = 5 - fullStars; // 빈 별의 개수를 구하기


    // 정수 부분의 평점만큼 채워진 별 아이콘을 생성하여 div에 추가
